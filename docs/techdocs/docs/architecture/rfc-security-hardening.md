...
> built*. The individual choices are recorded as [ADR-0007](adr-0007-cilium-wireguard-encryption.md)
> … [ADR-0009](adr-0009-secret-rotation-model.md) (with dynamic credentials split into its own
...
|---|----------|--------|--------|
| [ADR-0007](adr-0007-cilium-wireguard-encryption.md) | Wire encryption | Cilium **WireGuard** pod-to-pod, `nodeEncryption: false` | **Accepted** (shipped) |
| [ADR-0008](adr-0008-rootless-ci-image-builds.md) | CI build isolation | **Rootless BuildKit**, drop privileged DinD | **Proposed** (after runner proof) |
| [ADR-0009](adr-0009-secret-rotation-model.md) | Rotation model | OpenBao write → ESO refresh → Reloader restart | **Accepted** |
...
- **Supply-chain enforce** — promote the Kyverno image policies from `audit` → `enforce`. *Gated on*
  ...
  (see the [Forge migration](../../blog/2026-06-12-bringing-the-forge-home.md)); tracked there, not
  duplicated here.
...
- ADRs [0007](adr-0007-cilium-wireguard-encryption.md),
  [0008](adr-0008-rootless-ci-image-builds.md), [0009](adr-0009-secret-rotation-model.md)
- [RFC: Dynamic Database Credentials](rfc-dynamic-database-credentials.md) ·
  [RFC: Harbor Registry](rfc-harbor-registry.md)
- [Supply Chain Pipeline](../supply-chain-pipeline.md) · [Authentik](../authentik.md) ·
