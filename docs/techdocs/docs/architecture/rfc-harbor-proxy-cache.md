...
> down**. The individual decisions are [ADR-0016](adr-0016-harbor-pull-through-proxy-cache.md)
> (adopt the cache), [ADR-0017](adr-0017-registry-mirror-talos-spegel.md) (where the mirror is
> injected), and [ADR-0018](adr-0018-harbor-config-idempotent-job.md) (how the Harbor side is
...
- **Hosting webgrip's *own* images** (the `ghcr.io/webgrip/*` publish target). That is a separate
  ...
  here — Harbor is LAN-only ([ADR-0005](adr-0005-lan-only-exposure.md)), so GitHub-hosted Actions
  can't push to it.
- **Proxying the bootstrap/storage chain** (Talos, Cilium, CoreDNS, cert-manager, Longhorn CSI,
  ...
  upstream directly. Fallback makes this automatic — see [ADR-0017](adr-0017-registry-mirror-talos-spegel.md).
...
| # | Decision | Choice |
|---|----------|--------|
| [ADR-0016](adr-0016-harbor-pull-through-proxy-cache.md) | Adopt a pull-through cache | **Two Harbor proxy-cache projects**, `dockerhub` → docker.io and `ghcr` → ghcr.io |
| [ADR-0017](adr-0017-registry-mirror-talos-spegel.md) | Where to inject the mirror | **Talos `machine.registries.mirrors`** (per-registry, `overridePath`) composed with **Spegel `prependExisting: true`**; upstream fallback on |
| [ADR-0018](adr-0018-harbor-config-idempotent-job.md) | How to provision Harbor | **Idempotent CronJob** against the Harbor v2 API (no operator exists); creds via ESO/OpenBao |
...
Harbor's blobs live on **Garage S3** ([ADR-0002](adr-0002-registry-blob-storage-garage-s3.md)) —
the exact dependency whose outage cascaded into the
[2026-06-12 storage collapse](../blog/2026-06-13-harbor-as-a-pull-through-cache.md). Putting
...
- `harbor-proxy-config` CronJob + script — idempotently ensures the two registry endpoints and
  two proxy projects exist via the Harbor v2 API ([ADR-0018](adr-0018-harbor-config-idempotent-job.md)).
...
- Apply the Talos `machine.registries.mirrors` patch from
  [ADR-0017](adr-0017-registry-mirror-talos-spegel.md), one node at a time (reboot-safe drain via
  the `talos` skill).
...
- **CronJob noise.** Mitigated by the same `ttlSecondsAfterFinished` + history-limit hygiene the
  [openbao-init incident](../blog/2026-06-13-harbor-as-a-pull-through-cache.md) taught us.

## References

- ADRs [0016](adr-0016-harbor-pull-through-proxy-cache.md),
  [0017](adr-0017-registry-mirror-talos-spegel.md),
  [0018](adr-0018-harbor-config-idempotent-job.md)
- Extends [RFC: Harbor Container Registry](rfc-harbor-registry.md);
  [ADR-0002 Blob storage (Garage S3)](adr-0002-registry-blob-storage-garage-s3.md);
  [ADR-0005 LAN-only exposure](adr-0005-lan-only-exposure.md)
- [Harbor — Configure Proxy Cache][harbor-proxy] · [Spegel][spegel] ·
  [Talos RegistryMirrorConfig][talos-registries]
- [Platform Components](../platform-components.md) (Spegel) ·
  [Blog: Harbor as a pull-through cache](../blog/2026-06-13-harbor-as-a-pull-through-cache.md)
