...
([ADR-0029](adr-0029-dual-run-renovate-forgejo.md)): "Forgejo runs write-back CI for a repo" and
...
* Tags/releases diverge from GitHub for de-mirrored repos — intended, because GitHub is being
  ...
  push-mirrors, [ADR-0014](adr-0014-codeberg-offsite-push-mirror.md), built *after* cutover, from
  the new source.)
...
* **`homelab-cluster` follows the same path, last.** It is both the GitOps source and the one repo
  ...
  ([ADR-0011](adr-0011-flux-source-forgejo.md)) — cutover-last so the platform never loses its
  footing.
