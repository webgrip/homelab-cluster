...
With the node taxonomy in place ([ADR-0001](adr-0001-node-taxonomy.md)) and Longhorn confined to
the workers ([ADR-0008](adr-0008-confine-longhorn-to-workers.md)), placement must be deliberate.
...
* The PV-exclusion blocker is **StorageClass-specific, keyed on `volumeBindingMode`** — not all
  ...
  ([ADR-0010](adr-0010-storageclass-consolidation.md)). **Eviction does not help** — it cannot
...
* Apps are deterministically on the workers and the soyos host no Longhorn engine — completing
  ADR-0008's etcd-protection goal.
...
* Good, because placement is deterministic, and a stateful pod can never make a soyo run a
  Longhorn engine — completing [ADR-0008](adr-0008-confine-longhorn-to-workers.md)'s
...
* Bad, because rejected ([ADR-0001](adr-0001-node-taxonomy.md)): a taint blocks the infra that
