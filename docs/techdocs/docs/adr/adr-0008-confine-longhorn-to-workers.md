...
* Longhorn replicas only on worker-1 + fringe, enforced at the Longhorn layer
* Keep replicas on soyos, tune Longhorn harder
* k8s taints to keep Longhorn off the soyos
* A 3rd storage node now (3 replicas everywhere)
* A scoped soyo-replica exception for the GitOps root

## Decision Outcome

Chosen option: "Longhorn replicas only on worker-1 + fringe, enforced at the Longhorn layer",
because every storage SEV traces to Longhorn contending with etcd on the soyos' single shared
disk, and the structural fix is to take Longhorn off the soyos so etcd has its disk and RAM back.
...
`allowScheduling=false` — [ADR-0009](adr-0009-longhorn-hot-cold-tiers.md)'s node/disk mechanism),
...
([ADR-0012](adr-0012-external-bootstrap-fallback-source.md)).
...
* fringe's 236 GiB SSD is the binding constraint — depends on over-provisioning, cold-tiering
  ([ADR-0009](adr-0009-longhorn-hot-cold-tiers.md)), and a usage alert.
...
* Good, because it removes the root cause of the recurring storage SEVs — etcd gets its disk and
  RAM back on every soyo.
* Good, because enforcement lives in Longhorn's own scheduler (no schedulable disk,
  `allowScheduling=false`) — the layer that actually places replicas.
* Bad, because two storage nodes + hard anti-affinity give a **2-replica ceiling**.

### Keep replicas on soyos, tune Longhorn harder

* Bad, because every prior tuning round only relocated the contention; etcd sharing a disk with
  Longhorn is the defect.

### k8s taints to keep Longhorn off the soyos

* Bad, because replica placement is Longhorn's scheduler, not kube-scheduler; the clean lever is
  "no Longhorn disk on the node".

### A 3rd storage node now (3 replicas everywhere)

* Bad, because a hardware change; deferred, and the headline RFC follow-up.

### A scoped soyo-replica exception for the GitOps root

* Good, because a 3-replica `longhorn-gitops` class would keep forgejo/openbao serveable through a
  both-worker outage.
* Bad, because Garage S3 is external (off-cluster), so backups survive a both-worker outage
  anyway, and [ADR-0012](adr-0012-external-bootstrap-fallback-source.md) decouples forgejo's
  reconcile-resilience from its storage — initially adopted, dropped 2026-06-21 before being built
  (see Links).
...
* 2026-06-21 — exception dropped; the soyos stay 100% Longhorn-free. Garage S3 is external
  ...
  [ADR-0012](adr-0012-external-bootstrap-fallback-source.md) decouples forgejo's
