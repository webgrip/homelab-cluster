...
With Longhorn confined to the two workers ([ADR-0008](adr-0008-confine-longhorn-to-workers.md)),
...
* The soyos get **no** disk config → no schedulable Longhorn disk → ADR-0008's end-state is
  config, not a manual eviction that can drift back.

The hot tier is the default `longhorn` class itself ([ADR-0010](adr-0010-storageclass-consolidation.md));
...
[ADR-0007](adr-0007-storage-engine-gated-on-dedicated-disks.md).
...
* 2026-06-21 — the `gitops-critical` soyo disk + `longhorn-gitops` StorageClass retired unbuilt
  with the [ADR-0008](adr-0008-confine-longhorn-to-workers.md) update; the soyos get no Longhorn
  disk config at all
* 2026-07-01 — the fringe HDD wipe (the cold tier's prerequisite) deferred to the supervised disk
  step of [ADR-0007](adr-0007-storage-engine-gated-on-dedicated-disks.md); `longhorn-cold` remains
  inert and the annotation mechanism remains unbuilt
* 2026-07-03 — renumbered from ADR-0027 (pre-re-baseline numbering) in the layered re-ordering of the ADR set (see [index](index.md))
