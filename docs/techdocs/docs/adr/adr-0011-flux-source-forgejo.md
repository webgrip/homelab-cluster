...
Flux manages — is addressed by the companion [ADR-0012](adr-0012-external-bootstrap-fallback-source.md).
...
[ADR-0013](adr-0013-forgejo-leading-application-repos.md)'s cutover. Sequence and the gap-free
...
* Unblocks the Forgejo Renovate path adopting `homelab-cluster` and retiring the GitHub RenovateJob
  ([ADR-0029](adr-0029-dual-run-renovate-forgejo.md)), and sequences the Codeberg mirror
  ([ADR-0014](adr-0014-codeberg-offsite-push-mirror.md)).
...
* **Forgejo's availability becomes the cluster's ability to change itself.** While Forgejo is down,
  ...
  to Forgejo. Mitigation: [ADR-0012](adr-0012-external-bootstrap-fallback-source.md).
...
* Bad, because Forgejo runs *inside* the cluster Flux manages — the architectural weight addressed
  by [ADR-0012](adr-0012-external-bootstrap-fallback-source.md).
...
* Bad, because it leaves the cluster's control plane owned by a third party; retained only as the
  *fallback* ([ADR-0012](adr-0012-external-bootstrap-fallback-source.md)).
