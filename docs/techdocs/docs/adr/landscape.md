...
The ADR practice started **2026-06-12** (with what is now [ADR-0017](adr-0017-adopt-harbor.md);
the set was renumbered into layer order on 2026-07-03 — see the
[index](index.md#renumbering-2026-07-03)). Everything decided *after* that date is well
...
[ADR-0006](adr-0006-default-deny-network-policies.md) set the precedent for recording such
...
| --- | --- | --- | --- |
| Registry & artifacts | [Harbor registry](../rfc/rfc-harbor-registry.md) · [proxy cache](../rfc/rfc-harbor-proxy-cache.md) | 0017–0025 | Implemented |
| Security hardening (program) | [Security hardening](../rfc/rfc-security-hardening.md) | 0004, 0006, 0015, 0026 (Proposed) | Executing |
| Secrets | [external-secrets plan](../rfc/external-secrets-plan.md) · [dynamic DB creds](../rfc/rfc-dynamic-database-credentials.md) | 0015, 0016 | Plan done; pilot paused |
| Forge exit — Renovate | [Renovate on Forgejo](../rfc/rfc-renovate-forgejo.md) | 0029–0031 | Dual-run, retirement gated |
| Forge exit — GitOps source | [Flux → Forgejo](../rfc/rfc-flux-forgejo-source.md) | 0011, 0012, 0014 (all Proposed) | The big open cutover |
| Forge exit — repos & CI | [CI pipeline performance](../rfc/rfc-ci-pipeline-performance.md) | 0013, 0027, 0028 | Executing |
| TechDocs hosting | [Codeberg Pages](../rfc/rfc-codeberg-pages-techdocs.md) · [Backstage](../rfc/rfc-backstage-techdocs.md) | 0038, 0039 (Proposed) | Interim live; target unbuilt |
| Nodes & storage placement | [Node taxonomy](../rfc/rfc-node-taxonomy-and-storage-placement.md) | 0001, 0002, 0008–0010 | Implemented (0009/0010 tails open) |
| Hardware evolution (program) | [Layered hardware](../rfc/rfc-layered-hardware-architecture.md) | 0007 | Path not yet chosen |
| Alerting reliability | [Observability alerting](../rfc/rfc-observability-alerting-reliability.md) | 0035, 0036 | Implemented (delivery excluded — see gaps) |
| Policy / admission | [Kyverno audit→enforce](../rfc/rfc-kyverno-audit-enforce-hardening.md) | 0032, 0033 | Waves ongoing |
| Metrics backend | — (standalone) | 0034 | Implemented |
| Platform conventions | — (standalone) | 0003, 0005 (superseded), 0037 | Accepted |
...
| 9 | **The logs/traces/profiles pipeline is unrecorded.** Loki, Tempo, the two-Alloy collector topology, Beyla, retention tiers (30d/14d), single-replica-everything — only metrics (ADR-0034) has a record. | `observability/loki`, `tempo`, `alloy-*`, `beyla` | [Observability pipeline](../rfc/rfc-observability-pipeline.md) |
...
- **KEDA** — adopted as the forgejo-runner scaler; instrumental to ADR-0026, no independent decision surface.
- **Reloader / metrics-server / spegel** — single-purpose utilities; spegel is covered by ADR-0024, Reloader by ADR-0015.
