...
* The existing instances map cleanly: `openbao-init`, `github-app-token`, and `harbor-proxy-config`
  ([ADR-0025](adr-0025-harbor-config-idempotent-job.md)) are correct Tier-2 choices (volume wipe /
...
* **Honest tension with ADR-0025:** by the strict self-drift test, a Forgejo DB restore wipes the
...
* Bad, because the strict self-drift test leaves honest boundary cases (see the
  [ADR-0025](adr-0025-harbor-config-idempotent-job.md) tension under Negative Consequences).
...
* 2026-06-14 — accepted
* 2026-07-03 — renumbered from ADR-0019 (pre-re-baseline numbering) in the layered re-ordering of the ADR set (see [index](index.md))
