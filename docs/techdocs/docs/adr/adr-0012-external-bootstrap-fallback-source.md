...
[ADR-0011](adr-0011-flux-source-forgejo.md) makes the in-cluster Forgejo the steady-state GitOps
...
day-to-day (ADR-0011); **GitHub is retained — demoted to a Forgejo→GitHub push-mirror — as the
...
* **Codeberg** is planned as a *second* off-site push-mirror
  ([ADR-0014](adr-0014-codeberg-offsite-push-mirror.md)), so the DR source isn't itself a single host.
