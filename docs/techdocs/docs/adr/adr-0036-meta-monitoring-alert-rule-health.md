...
The SLO rules errored for ~3 weeks ([ADR-0035](adr-0035-grafana-threshold-rule-shape.md)) and
...
1. **Scrape Grafana's `/metrics`.** The ServiceMonitor's `.spec.selector` matches the labels the
   ...
   [ADR-0034](adr-0034-victoriametrics-metrics-backend.md) VictoriaMetrics backend, the vm-operator
...
2. **A page-on-failure meta-rule.** `slo-grafana-alert-rule-eval-failing` in the
   ...
   **`noDataState: Alerting` and `execErrState: Alerting`** — a watcher that itself loses data or
   errors must page, not go silent. It uses the canonical `expression: query` shape from ADR-0035,
...
* Good, because the meta-rule uses the canonical `expression: query` shape from ADR-0035, so it
...
* 2026-07-01 — metrics backend swapped to VictoriaMetrics
  ([ADR-0034](adr-0034-victoriametrics-metrics-backend.md)): the ServiceMonitor is now
...
* 2026-07-03 — renumbered from ADR-0031 (pre-re-baseline numbering) in the layered re-ordering of the ADR set (see [index](index.md))
