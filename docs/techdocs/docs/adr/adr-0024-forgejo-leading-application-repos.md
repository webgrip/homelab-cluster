...
- **Keep the read-only mirror; cut releases only on GitHub** — keeps GitHub load-bearing for the
  release step; the opposite of the migration's north star.
- **Writable mirror with bidirectional sync** — tag divergence + clobbering; the fragile two-way
  sync pull-mirrors exist to avoid.
- **GitHub→Forgejo *push*-mirror (GitHub pushes; Forgejo writable)** — workable, but keeps GitHub
  the source; the goal is to archive GitHub, not re-plumb it.
