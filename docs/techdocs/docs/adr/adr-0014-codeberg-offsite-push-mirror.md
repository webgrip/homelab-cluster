...
[ADR-0012](adr-0012-external-bootstrap-fallback-source.md) names Codeberg as a planned *second*
...
self-drift [ADR-0025](adr-0025-harbor-config-idempotent-job.md) calls out), so coverage must
...
([ADR-0011](adr-0011-flux-source-forgejo.md)) — until Forgejo is authoritative, most `webgrip/*`
...
([ADR-0003](adr-0003-bootstrap-task-pattern.md)) running in-cluster against the two Forgejo APIs.
...
last-sync/health is alerted (ADR-0012's "keep the push-mirror honest"), so a silent mirror failure
