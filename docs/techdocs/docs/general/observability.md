...
- Endpoint `vtsingle-victoriatraces.observability.svc:10428` — OTLP/HTTP ingest under `/insert/opentelemetry`, **Jaeger query API** under `/select/jaeger`, and (since VT v0.9.4, 2026-07-12) the experimental **Tempo query API with TraceQL** under `/select/tempo`.
...
- Grafana: **two datasources, one backend** — uid `victoriatraces` (type `jaeger`) for dashboard panels (Jaeger `search` queries), and uid `victoriatraces-tempo` (type `tempo`) serving the **Traces Drilldown** app + TraceQL (experimental; exemplar and log-derived trace links point here). Trace-to-logs → `victorialogs`, trace-to-metrics → span metrics, service map (tempo DS) → `prometheus`.
- **Drilldown coverage note (2026-07-12):** Metrics Drilldown works against uid `prometheus`; Traces Drilldown against uid `victoriatraces-tempo`; **Logs Drilldown cannot work with VictoriaLogs** — the app is hard-wired to loki-type datasources and Loki-only APIs (upstream: [victorialogs-datasource#424](https://github.com/VictoriaMetrics/victorialogs-datasource/issues/424), [VictoriaLogs#454](https://github.com/VictoriaMetrics/VictoriaLogs/issues/454), grafana/logs-drilldown#1597). Fallback = the `victorialogs-explorer` dashboard; support arrives automatically when upstream ships it (the plugin is unpinned in `GF_INSTALL_PLUGINS`).
