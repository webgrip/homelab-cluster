...
- **Mass `Error/137` with identical timestamps across namespaces → one node event, not N app bugs.** Group by `spec.nodeName` first. On Talos it's usually the **userspace OOM controller** (PSI-triggered, kills largest **BestEffort** cgroups first — kernel `node_vmstat_oom_kill` stays flat): check `talosctl -n <ip> get oomactions` (the kill ledger; dmesg may be useless — AVC-flooded). Fix is QoS (give the victims memory requests), and mind the two request axes: this pool has been memory-request-full on one node and CPU-request-full on the other — adding CPU requests left 7 DBs Pending (incidents/2026-07-11-talos-oom-db-tier.md).
