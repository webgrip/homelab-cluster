...
- **Zero-trust namespace?** the DB-layer ks needs `components/cnpg-netpol` or cnpg-system can't poll the instance :8000 → `ClusterIsNotReady` deadlock — see the `network-policy` skill. The **barman-cloud plugin also needs apiserver access** (reads the `ObjectStore` CRD): without it WAL archiving dies **silently** — `ContinuousArchiving=True` keeps showing while `pg_wal` fills toward a crash (2026-07-17: 5Gi in ~1.5 days). Verify archiving via recent `backups.postgresql.cnpg.io` / WAL objects in the bucket, never via that condition.
