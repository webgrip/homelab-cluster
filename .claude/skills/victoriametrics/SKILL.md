...
## Upgrading the stack = one chart bump

No CR pins an image: **all six** data-plane workloads (vmsingle, vmagent, vmalert, vmalertmanager, vlsingle, vtsingle) run the vm-operator chart's default images, so bumping `vm-operator/app/ocirepository.yaml` `ref.tag` upgrades everything in one reconcile (verified 0.65.1→0.66.2: VM v1.147.0, VL v1.51.0, VT v0.9.4, alertmanager v0.31.0). Chart version ≠ operator version (chart 0.65.1 shipped operator v0.72.0). Consequences: backend **breaking changes ride along invisibly** — check the VL changelog for LogsQL breaks (see victorialogs skill's v1.51 gotcha) and the VT/VM changelogs before bumping; refresh the OCI digest; leave `serviceMonitor.enabled: false` alone (rule above). Operator v0.73.0's RBAC tightening (namespace-scoped config-reloader Roles) needed no CR/values changes.
...
Consumers to grep for (endpoint string, not just `dependsOn`): the GrafanaDatasource CRs, OpenCost `prometheus.internal.serviceName`, alloy-gateway/k6 `remote_write`. (The Grafana chart HelmRelease that duplicated the datasources was deleted 2026-07-12.) Keep datasource `uid: prometheus`/`alertmanager`, change only the URL → dashboards need no edits.
