...
[ADR-0025](docs/techdocs/docs/adr/adr-0025-node-taxonomy.md)/[ADR-0028](docs/techdocs/docs/adr/adr-0028-application-workload-placement.md).
Storage direction: Longhorn replicas off the soyos (`longhorn` skill, [ADR-0026](docs/techdocs/docs/adr/adr-0026-confine-longhorn-to-workers.md)).
...
- **Adding a node ≠ existing stateful workloads can use it.** `longhorn-general` (WFFC) bakes each PV's
  `nodeAffinity` at first bind → **excludes** the new node (worker-1); `longhorn` (Immediate) sets none →
  fine anywhere. New volumes are always fine. Full rule + migration fix → the `workload-placement` skill.
