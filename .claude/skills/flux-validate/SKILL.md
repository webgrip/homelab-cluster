...
- **Applied ≠ effective — the second blind spot.** A Kustomize component can clobber fields your manifest sets: `components/placement/worker-pool` replaces `spec.postRenderers` **wholesale**, which left guac's resource patches silently inert for 3 days while everything rendered and reconciled green (2026-07-15→18). After merge + reconcile, live-verify the **pods** (resources/affinity actually present), not just HelmRelease `Ready`.
- **Values set but the chart never renders them?** Pull the pinned chart and grep the template before suspecting a values-path typo: `helm pull <repo>/<chart> --version <v> --untar && grep -rn resources <chart>/templates/` — e.g. guac 0.8.0's rest-api/visualizer templates never reference `.resources` at all (fix = postRenderers, not values).
- **Clean-pass output is terse (~3 lines).** Success = exit 0 + the `kustomizations=<count>` completion line — don't mistake brevity for a skipped run.
