...
2. Confirm backups are configured and recent:
   ...
   - On the barman-cloud plugin path the Cluster's `.status.lastSuccessfulBackup` does NOT populate — judge from the `Backup` CRs (`PHASE=completed`) and `firstRecoverabilityPoint`, or you'll report a false 🔴.
   - Verify the Garage/S3 WAL archive target is healthy — a stale target is the SPOF (see `docs/techdocs/docs/runbooks/cnpg-backups.md`).
